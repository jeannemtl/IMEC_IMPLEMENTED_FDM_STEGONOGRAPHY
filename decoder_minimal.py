"""
MINIMAL TEST DECODER - Official iMEC

Decodes the output from encoder_minimal.py to verify:
1. iMEC decoding works
2. Message recovery is accurate
3. Complete pipeline functions correctly
"""
...
def plot_fft_analysis(signal, sample_rate, title="FFT Analysis"):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('fft_analysis_minimal.png', dpi=150)
    print(f"✓ Saved FFT plot to: fft_analysis_minimal.png")


def main():
    print("="*80)
    print("MINIMAL TEST DECODER - Official iMEC")
    print("="*80)
    ...
    input_file = 'encoded_data_minimal.pkl'
    try:
        ...
        print("Please run encoder_minimal.py first.")
        sys.exit(1)
    ...
    print(f"\nInitializing iMEC decoder (block_size={block_size_bits})...")
    decoder = IMECDecoder(
    ...
    decrypted_bitarray = decoder.decode(
        stegotext=imec_tokens,
        context=context,
        num_chunks=n_chunks,
        verbose=True
    )
    ...
    print(f"\n✓ iMEC decoding complete in {elapsed:.1f} seconds!")
    print(f"  Recovered: {len(decrypted_bitarray)} bits")
    ...
    print("\n" + "="*80)
    print("STEP 2: DECRYPTION (REMOVE MASK)")
    print("="*80)
    
    signal_bitarray = remove_random_mask(decrypted_bitarray, **mask_cfg)
    
    # Convert to string
    signal_bits = signal_bitarray.to01()
    
    print(f"✓ Decryption complete:")
    print(f"  Decrypted bits: {len(signal_bits)}")
    ...
    recovered_signal = reconstruct_from_bits(signal_bits, quant_metadata)
    ...
    if 'combined_signal' in encoded_data:
        ...
        if rmse < 0.1:
            ...
            print(f"  ⚠️  Significant error in reconstruction")
    ...
    print(f"\nPipeline summary:")
    print(f"  1. iMEC decode:    {len(imec_tokens)} tokens → {len(decrypted_bitarray)} bits")
    print(f"  2. Decrypt:        {len(signal_bits)} bits recovered")
    print(f"  3. Reconstruct:    {len(recovered_signal)} samples")
    print(f"  4. FFT extract:    3 agents processed")
    ...
    if overall_accuracy >= 0.9:
        ...
        print("Complete pipeline verified: FDM → Quantize → Encrypt → iMEC → Decode → Recover")
    elif overall_accuracy >= 0.75:
        print("\n⚠️  PARTIAL SUCCESS: Most messages recovered")
    else:
        print("\n❌ FAILURE: Poor message recovery - check signal parameters")
